class Play extends Phaser.Scene {
    ...
    preload() {
        ...
        this.load.spritesheet('goldex', './assets/goldex.png', { frameWidth: 16, frameHeight: 8, startFrame: 0, endFrame: 7 });
    }
    ...
    create() {
        ...
        //add gold bar
        this.goldie = new Goldbar(this, game.config.width + borderUISize * 12, borderUISize * 3.5, 'goldbar', 0, 100).setOrigin(0, 0);
        ...
        this.anims.create({
            key: 'goldex',
            frames: this.anims.generateFrameNumbers('goldex', { start: 0, end: 7, first: 0 }),
            frameRate: 20
        });
    ...
    update() {
        ...
        // moves and updates the tile sprite (4 pixel at a time)
        ...
        if (!this.gameOver) {
            ...
            this.ship02.update();
            this.ship03.update();
            this.goldie.update();           // update the goldbar
        }
        ...
        // check collisions for the goldbar
        if (this.checkCollision(this.p1Rocket, this.goldie)) {
            this.p1Rocket.reset();
            this.goldExplode(this.goldie);
        }
    }
    ...
    goldExplode(ship) {
        ...
        ship.alpha = 0;
        // create explosion sprite at ship's position
        let boom = this.add.sprite(ship.x, ship.y, 'goldex').setOrigin(0, 0);
        boom.anims.play('goldex');             // play explode animation
        boom.on('animationcomplete', () => {    // callback after ani completes
            ship.reset();                       // reset ship position
            ship.alpha = 1;                     // make ship visible again
            boom.destroy();                     // remove explosion sprite
